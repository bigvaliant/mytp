...
//        Hook::listen('ems_get', $ems, null, true);
    ...
    public static function send($email, $code = NULL, $event = 'default')
    {
        ...
        $time = time();
        $ip = request()->ip();
        $ems = \app\base\model\Ems::create(['event' => $event, 'email' => $email, 'code' => $code, 'ip' => $ip]);
    ...
    public static function check($email, $code, $event = 'default')
    {
        ...
        $ems = \app\base\model\Ems::where(['email' => $email, 'event' => $event])
                ->order('id', 'DESC')
                ->find();

        if ($ems)
        {
            if (strtotime($ems['createdTime']) > $time && $ems['times'] <= self::$maxCheckNums)
            {
...
//                    \app\common\model\Ems::destroy($ems->id);
                    return $ems->id;
                }
            }
            else
